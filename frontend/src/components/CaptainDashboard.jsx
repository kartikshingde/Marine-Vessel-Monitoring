...
// ✅ Create Ship Icon Marker - No Circle
...
const CaptainDashboard = () => {
  ...
  useEffect(() => {
    ...
    // ✅ Listen for sensor updates with position
    newSocket.on("sensor-update", (data) => {
      if (data.vesselId === vessel._id && data.position) {
        setVessel((prev) => ({
          ...prev,
          currentPosition: data.position,
          speed: data.sensors?.speed || prev.speed,
          heading: data.sensors?.heading || prev.heading,
        }));
      }
    });
          ...
          <div className="p-6">
            <div className="h-64 rounded-xl overflow-hidden border-2 border-gray-200">
              <MapContainer
                ...
                key={`${vessel.currentPosition?.latitude}-${vessel.currentPosition?.longitude}`}
              >
                <TileLayer
                ...
                <Marker
                  ...
                  icon={createShipIcon(vessel.heading || 0)}
                >
                  <Popup>
                    <div className="p-2">
                      ...
                      <div className="text-sm space-y-1">
                        <p>
                          ...
                          {vessel.currentPosition?.latitude.toFixed(4)}°,{" "}
                          {vessel.currentPosition?.longitude.toFixed(4)}°
                        </p>
                        <p>
                          <strong>Speed:</strong>{" "}
